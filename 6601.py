...
def draw_collected_apples():
    ...
    glBegin(GL_QUADS)
    glTexCoord(0, 0)
    glVertex(0+(20*(current_player_result-1)), 0, 0)
    glTexCoord(1, 0)
    glVertex(0+(20*(current_player_result)), 0, 0)
    glTexCoord(1, 1)
    glVertex(0+(20*(current_player_result)), 20, 0)
    glTexCoord(0, 1)
    glVertex(0+(20*(current_player_result-1)), 20, 0)
...
def check_apple_box_in_touch(_apple, _player):
    ...
    #horizontal_check = _player.left <= _apple.right <=_player.right or _player.left <=_apple.left  <=_player.right
...
def lose():
    ...
    glLineWidth(2)
    string1 ="Game over"
    draw_text(string1, 390, 250)
    #string = "press q to exit"
    #draw_text(string, 330, 400)
    SPEED=-1
...
def win_game():
    ...
    glColor(0, 1, 0)
    glLineWidth(1.5)
    string1 = "you win , congratulation"
    draw_text(string1, 350, 250)
    #string = "press q to exit"
    #draw_text(string, 330, 400)
    SPEED = -1
...
def Run_game():
    ...
    if(START==-1):  #to stop game
        ...
        draw_Moving_Box(current_player)
    glLoadIdentity()
    for i in range(current_player_result):
        glBindTexture(GL_TEXTURE_2D, texture[1])
        glBegin(GL_QUADS)
        glTexCoord(0, 0)
        glVertex(0 + (17.7 * (i )), 0, 0)
        glTexCoord(1, 0)
        glVertex(0 + (17.7 * (i+1)), 0, 0)
        glTexCoord(1, 1)
        glVertex(0 + (17.7 * (i+1)), 17.7, 0)
        glTexCoord(0, 1)
        glVertex(0 + (17.7 * (i )), 17.7, 0)
        glEnd()
...
def main():
    ...
    window=glutCreateWindow(b"!!! catch the apple !!! -- Team 10")
